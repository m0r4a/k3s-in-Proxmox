...
│   ├── versions.tf             # Provider pinning (bpg/proxmox)
│   ├── providers.tf            # Provider config (endpoint, token, ssh)
...
└── ansible/
    ...
    ├── inventory-example.ini   # Standalone example inventory
    ├── group_vars/
...
The connection point is `inventory.ini`, generated by terraform with
infrastructure facts only: IPs, users, SSH key path, control plane IP,
cluster CIDRs. All ansible configuration (cilium versions, install URLs,
reboot policy) lives in committed `ansible/group_vars/all.yml`.

Ansible can run standalone with a manual inventory — it does not depend
on terraform at runtime, only on the inventory file format.
...
1. `cd terraform && terraform destroy` — cluster gone.
2. Change FluxCD target / node IPs / node counts in `main.tf`.
3. `cd terraform && terraform apply` — new cluster up.
4. `cd ../ansible && ansible-playbook -i inventory.ini site.yml`
